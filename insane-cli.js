  ...
  if (argv4 != null) {
    ...
    if (argv3 === "-url" || argv3 === "/url") {
      ...
      request(url, function (error, response, body) {
        ...
        if (
          Object.keys(archived_snapshots).length === 0 &&
          archived_snapshots.constructor === Object
        ) {
          ...
          console.log(
            `Archived Time: ${year}-${month}-${day} ${hour}-${min}-${sec}`
          );
        }
      });
    ...
    if (argv3 === "--version" || argv3 === "/v") {
      ...
      // for local file process, read and process
      let filename = argv3;
