    ...
    if (argv3 === "-url" || argv3 === "/url") {
      request(argv4, function (error, response, body) {
        if (response.statusCode === 200) {
          // only process url with statusCode 200
...
function getURLs(data) {
  ...
  for (let i = tempArr.length; i--; ) {
    let tmp =
      (tempArr[i].match(PT1) || tempArr[i].match(PT2)) != null
...
function getUrl(url) {
  return new Promise((resolve, reject) => {
    try {
      request(url, { method: "HEAD", timeout: 3000 }, function (
        error,
        response,
        body
      ) {
        ...
        if (status !== null) {
          if (status === 200) {
            console.log("[" + status + "]GOOD - " + url.green);
          } else if (status === 400 || status === 404) {
            console.log("[" + status + "]BAD - " + url.red);
          } else if (status === 301 || status === 307 || status === 308) {
            console.log("[" + status + "]REDIRECT - " + url.blue);
          } else {
            console.log("UNKNOWN - " + url.grey);
          }
        }
        resolve();
      });
    } catch (error) {
      console.error("WTF - " + url.yellow);
    }
  });
}
...
function showHelp() {
  ...
  console.log(
    " insane-cli [--version][/v]".green + " to check current version".blue
  );
  console.log(" insane-cli [file-path]".green + " process a file".blue);
  console.log(
    " insane-cli -url [full-url-link]".green + " process body's link".blue
  );
}
